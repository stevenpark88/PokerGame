...
namespace UIAccount
{
	public class ProfileTimeZone : MonoBehaviour
	{
		#region PUBLIC_VARIABLES

		public Toggle toggleUsernameShow;
		public Toggle toggleUsernameHide;

		public Toggle toggleContactDetailsShow;
		public Toggle toggleContactDetailsHide;

		public Toggle toggleSoundOn;
		public Toggle toggleSoundOff;

		public Dropdown dropdownTimezone;

		#endregion

		#region PRIVATE_VARIABLES

		private List<string> timeZoneKeysList;
		private List<string> timeZoneValuesList;

		private bool isDetailReceived;
		...
		void OnEnable ()
		{
			AccountAPIManager.onAccountDetailReceived += OnAccountDetailReceived;
			AccountAPIManager.onAccountDetailUpdated += OnAccountDetailUpdated;

			AccountAPIManager.GetInstance ().GetAccountDetail (!isDetailReceived);
		}
		...
		private void OnAccountDetailReceived (string info)
		{
			isDetailReceived = true;

			UIManager.Instance.HideLoader ();

			AccountInfo accountInfo = JsonUtility.FromJson<AccountInfo> (info);

			if (accountInfo.contact) {
				toggleContactDetailsShow.isOn = true;
			} else {
				toggleContactDetailsHide.isOn = true;
			}

			if (accountInfo.sound) {
			...
			if (accountInfo.username) {
				toggleUsernameShow.isOn = true;
			} else {
				toggleUsernameHide.isOn = true;
			}

			DataManager.IsSoundEnabled = accountInfo.sound;
			dropdownTimezone.value = timeZoneKeysList.IndexOf (accountInfo.time_zone);
		}

		private void OnAccountDetailUpdated (string info)
		{
			UIManager.Instance.HideLoader ();
		}
		...
		{
			int username_val = toggleUsernameShow.isOn ? 1 : 0;
			int contact_val = toggleContactDetailsShow.isOn ? 1 : 0;
			int sound_val = toggleSoundOn.isOn ? 1 : 0;
			string timezone_val = timeZoneKeysList [dropdownTimezone.value];
			
			DataManager.IsSoundEnabled = toggleSoundOn.isOn;
			AccountAPIManager.GetInstance ().SetAccountDetail (username_val, contact_val, sound_val, timezone_val);
		}
